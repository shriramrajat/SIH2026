...
def test_analysis_failure(monkeypatch, temp_repo):
    ...
    monkeypatch.setattr("ecdat.service.classify_assets", mock_classify)
...
# â”€â”€ P1.4 / P1.5: health and version endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ P0.4: evidence traceability â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def test_evidence_traceability(temp_repo):
    response = client.post("/api/v1/scan", json={"target_path": str(temp_repo)})
    assert response.status_code == 200
    data = response.json()
    for finding in data["findings"]:
        ...
        assert isinstance(ev["line_number"], int) and ev["line_number"] > 0
        assert "code_snippet" in ev
        assert "detection_mechanism" in ev
        assert "matched_rule_id" in ev


# â”€â”€ P0.5: summary integrity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def test_summary_includes_discovery_counts(temp_repo):
    response = client.post("/api/v1/scan", json={"target_path": str(temp_repo)})
    data = response.json()
    summary = data["summary"]
    assert "total_files_discovered" in summary
    assert "total_files_scanned" in summary
    assert "files_skipped" in summary
    assert "files_failed" in summary
    assert summary["total_files_discovered"] >= summary["total_files_scanned"]


# â”€â”€ P1.2: response includes key_length / mode / padding â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def test_response_includes_optional_asset_fields(temp_repo):
    response = client.post("/api/v1/scan", json={"target_path": str(temp_repo)})
    data = response.json()
    for finding in data["findings"]:
        # These keys must always be present (may be null)
        assert "key_length" in finding
        assert "mode" in finding
        assert "padding" in finding


# â”€â”€ response structure completeness â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def test_response_structure_completeness(temp_repo):
    response = client.post("/api/v1/scan", json={"target_path": str(temp_repo)})
    assert response.status_code == 200
    data = response.json()
    assert "summary" in data
    ...
    assert "metadata" in data
    assert "scan_duration_ms" in data["metadata"]
